...
- Local packaging orchestration for packaged VKEN Design Engine artifacts.
...
- Release artifacts keep canonical app identity (`VKEN Design Engine.app` on mac, `VKEN Design Engine.exe` inside the Windows installer); local tools-pack installs may use namespace-scoped install paths only as a developer multi-instance validation convention.
