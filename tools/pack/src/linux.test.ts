...
Type=Application
Name=VKEN Design Engine (@@NAMESPACE@@)
...
`;
  ...
  it("substitutes all @@TOKEN@@ placeholders", () => {
    const out = renderDesktopTemplate(template, {
      namespace: "default",
      execPath: "/home/u/.local/bin/VKEN-Design-Engine.default.AppImage",
      iconName: "open-design-default",
    });
    expect(out).toContain("Name=VKEN Design Engine (default)");
    expect(out).toContain(
      "Exec=env OD_PACKAGED_NAMESPACE=default /home/u/.local/bin/VKEN-Design-Engine.default.AppImage --appimage-extract-and-run %U",
    );
...
describe("matchesAppImageProcess", () => {
  const installPath = "/home/u/.local/bin/VKEN-Design-Engine.default.AppImage";
  ...
  it("matches --appimage-extract-and-run mode (executable in /tmp/appimage_extracted_*/<binary>)", () => {
    const ok = matchesAppImageProcess(
      {
        pid: 1234,
        executable: "/tmp/appimage_extracted_fe548e54/VKEN Design Engine",
