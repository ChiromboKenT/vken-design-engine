...
Local packaging control plane for VKEN Design Engine.
...
Release artifacts keep the canonical `VKEN Design Engine.app` bundle shape; local `tools-pack install` copies it as
`VKEN Design Engine.<namespace>.app` so developer namespaces can coexist without affecting runtime data/log/cache paths.
...
- AppImage: `~/.local/bin/VKEN-Design-Engine.<namespace>.AppImage`
