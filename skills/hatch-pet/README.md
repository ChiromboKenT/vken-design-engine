...
checked into the VKEN Design Engine repo (rather than pulled in as a Git submodule
or an npm package) so that:

- Any VKEN Design Engine agent can run the skill end-to-end without a network
...
- Pinned upstream reference (declared in `SKILL.md` frontmatter): see the
  ...
  directory as the authoritative source-of-truth for VKEN Design Engine and
...
2. Replace the contents of this directory with the upstream snapshot,
   preserving only this `README.md` and any VKEN-specific notes
   inside `SKILL.md`'s `> **VKEN Design Engine integration.**` blockquote.
...
flow (including how VKEN Design Engine behaves when Codex is not installed).
