/**
 * vken-landing-deck — input schema.
 *
 * Sister skill to `vken-landing`. Produces a single-file slide
...
import type { MixedText, BrandBlock, ImageryConfig } from '../vken-landing/schema';
...
export interface CoverSlide {
  kind: 'cover';
  /** Eyebrow above the title — `'VKEN Design Engine · Vol. 01'`. */
  eyebrow: string;
...
export interface EndSlide {
  kind: 'end';
  /** The huge kicker — `'VKEN Design Engine.'`. */
  mega: MixedText;
...
export interface VkenLandingDeckInputs {
  $schema?: string;
  brand: BrandBlock;
  /** Deck-wide title shown in the HUD — `'VKEN Design Engine · Vol. 01'`. */
  deck_title: string;
  slides: Slide[];
...
/**
 * @deprecated Use `VkenLandingDeckInputs`.
 *
 ...
 */
export type EditorialCollageDeckInputs = VkenLandingDeckInputs;
