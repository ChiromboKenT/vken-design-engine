...
description: >
  ...
  dots, surreal collage plates) — VKEN Design Engine's brand deck recipe.
...
inputs:
  ...
  - id: deck_title
    ...
    description: e.g. `'VKEN Design Engine · Vol. 01 / Issue Nº 26'`.
  - id: slides
  ...
  - id: imagery
    ...
    schema_path: ../vken-landing/schema.ts#ImageryConfig
parameters:
...
example_prompt: |
  ...
  card. Reuse the vken-landing image library.
---

# vken-landing-deck

Sister skill to [`vken-landing`](../vken-landing/). Same
...
```text
inputs.json + ../vken-landing/styles.css
        │
        └──────────► scripts/compose.ts
                            │
                            ▼
...
`vken-landing` brief, copy `brand` and `imagery` over verbatim.
...
script — it shares the 16-slot library from `vken-landing`. To
regenerate or stub:

```bash
# generate via gpt-image-2 (fal.ai)
FAL_KEY=... npx tsx ../vken-landing/scripts/imagegen.ts ../vken-landing/inputs.example.json --out=../vken-landing/assets/
...
live (default in the example: `../vken-landing/assets/`).
...
stylesheet from `../vken-landing/styles.css`, layers
...
- **Reuse the sister skill's stylesheet.** The composer reads
  `../vken-landing/styles.css` at compile time. Do not
