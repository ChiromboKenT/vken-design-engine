...
/**
 * vken-landing-deck — slide deck composer.
 *
 ...
 * stylesheet from the sister `vken-landing` skill, then layers
...
const SISTER_STYLES = resolve(SKILL_ROOT, '..', 'vken-landing', 'styles.css');
...
/* ------------------------------------------------------------------ *
 * deck-specific stylesheet (layered on top of vken-landing CSS).
 *
...
function chromeStrip(brand: VkenLandingDeckInputs['brand'], deckTitle: string): string {
...
function footStrip(idx: number, total: number, brand: VkenLandingDeckInputs['brand']): string {
...
(function () {
  ...
  function applySlide(n) {
    ...
    /* load-bearing: .slide.active is read by VKEN Design Engine's host bridge
  ...
  /* Host-driven navigation: VKEN Design Engine's host bridge classifies this deck
...
export function renderDeck(inputs: VkenLandingDeckInputs, baseCss: string): string {
...
async function main(): Promise<void> {
  ...
  const inputs = JSON.parse(inputsRaw) as VkenLandingDeckInputs;
