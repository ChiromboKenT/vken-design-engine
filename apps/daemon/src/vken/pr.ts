...
import type { VkenValidationResult, VkenValidationStage } from './validate.js';
...
export async function createVkenPullRequest(input: {
  ...
  patches: VkenPatch[];
  validation: VkenValidationResult;
  bundleUrl: string;
  replayUrl: string;
}): Promise<string> {
...
### Validation
${renderValidationChecklist(input.validation.byStage)}
...
export function renderValidationChecklist(byStage: Partial<VkenValidationResult['byStage']>): string {
  const line = (label: string, stage: VkenValidationStage) => {
    const s = byStage[stage];
    if (!s) return `- WARN ${label} (not run)`;
    const marker = s.ok ? 'PASS' : 'FAIL';
    return `- ${marker} ${label}${renderStageDetail(stage, s)}`;
  };
  return [
    line('tsc --noEmit', 'tsc'),
...
function renderStageDetail(stage: VkenValidationStage, s: { ok: boolean; details: unknown }): string {
  const details = isRecord(s.details) ? s.details : {};
  if (stage === 'a11y' && typeof details.violationCount === 'number') {
    return ` (${details.violationCount} violations, ${Number(details.seriousCount ?? 0)} serious)`;
  }
  if (stage === 'pixel' && typeof details.visualGap === 'number') {
    const threshold = typeof details.threshold === 'number' ? details.threshold : 0.2;
    return ` (gap ${details.visualGap.toFixed(3)} / ${threshold})`;
  }
  if (stage === 'console' && Array.isArray(details.errors)) {
    return ` (${details.errors.length} errors)`;
  }
  if (typeof details.skipped === 'string') return ` (skipped: ${details.skipped})`;
